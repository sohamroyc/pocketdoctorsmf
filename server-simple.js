...
app.use((req, res, next) => {
  res.setHeader('Content-Security-Policy', "default-src 'self'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://cdnjs.cloudflare.com; script-src 'self' 'unsafe-inline' 'unsafe-eval'; img-src 'self' data: https: blob:; font-src 'self' data: https://fonts.gstatic.com https://cdnjs.cloudflare.com; connect-src 'self' https://generativelanguage.googleapis.com;");
  next();
});
...
const apiKey = process.env.GEMINI_API_KEY || 'AIzaSyA4k84hnDhRJZMlOlukYM6uM8iPAPVKrQ0';

if (!apiKey || apiKey === 'YOUR_GEMINI_API_KEY_HERE') {
  console.warn('❌ GEMINI_API_KEY is not set or invalid. AI features will not work.');
  console.log('📝 Please get a valid API key from: https://aistudio.google.com/');
  console.log('📝 Then update the .env file or replace YOUR_GEMINI_API_KEY_HERE in server-simple.js');
} else {
  console.log('✅ GEMINI_API_KEY is set and ready to use');
  console.log('🤖 AI Chatbot and Symptom Checker are now active!');
}

// Health-focused fallback responses when API is not available
const getFallbackResponse = (message) => {
  const messageLower = message.toLowerCase();
  
  // Health-specific responses based on keywords
  if (messageLower.includes('fever') || messageLower.includes('temperature')) {
    return "A fever can indicate infection. Monitor your temperature and stay hydrated. If fever persists or is high, consult a healthcare provider immediately.";
  }
  
  if (messageLower.includes('headache') || messageLower.includes('head pain')) {
    return "Headaches can have various causes. Rest in a quiet, dark room and stay hydrated. If severe or persistent, seek medical attention.";
  }
  
  if (messageLower.includes('cough') || messageLower.includes('cold')) {
    return "For cough and cold symptoms, rest, stay hydrated, and use a humidifier. If symptoms worsen or persist, consult a healthcare provider.";
  }
  
  if (messageLower.includes('pain') || messageLower.includes('ache')) {
    return "Pain management depends on the cause and location. Rest the affected area and consider over-the-counter pain relief. Consult a doctor if pain is severe or persistent.";
  }
  
  if (messageLower.includes('stomach') || messageLower.includes('nausea') || messageLower.includes('vomit')) {
    return "Stomach issues require careful monitoring. Stay hydrated with clear fluids and avoid solid foods initially. Seek medical help if symptoms worsen.";
  }
  
  if (messageLower.includes('breathing') || messageLower.includes('chest') || messageLower.includes('shortness')) {
    return "Breathing difficulties require immediate attention. If you're experiencing severe shortness of breath or chest pain, seek emergency medical care right away.";
  }
  
  // General health response
  const generalResponses = [
    "I'm here to help with health-related questions. For specific medical concerns, please consult with a healthcare professional for proper evaluation and treatment.",
    "Your health is important. While I can provide general wellness information, please see a doctor for proper medical assessment of your symptoms.",
    "I understand your health concerns. For accurate diagnosis and treatment, it's best to consult with a qualified healthcare provider.",
    "Thank you for sharing your health question. For proper medical advice, please schedule an appointment with your healthcare provider."
  ];
      ...
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({
          systemInstruction: {
            parts: [
              {
                text: 'You are a professional AI Health Assistant. ONLY provide health-related advice, medical information, and wellness guidance. Respond in EXACTLY 2-3 short sentences maximum. Use plain text only—no bullets, no lists, no asterisks, no markdown. Keep responses concise and direct. If symptoms may be serious, include a brief safety note. If asked about non-health topics, politely redirect to health-related questions.'
              }
            ]
          },
          generationConfig: {
...
app.post('/api/analyze-symptoms', async (req, res) => {
  try {
    ...
    const prompt = `You are a professional medical AI assistant specializing in symptom analysis. ONLY provide health-related assessments and medical guidance. Analyze the following symptoms and provide a concise response in this exact JSON format:

{
  "analysis": "Brief 2-3 sentence health-focused analysis of the symptoms. Always recommend consulting a healthcare provider for proper diagnosis.",
